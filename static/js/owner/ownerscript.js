...
function editBungalow(id) {
    // Implement the edit functionality here
    console.log('Edit bungalow with ID:', id);
}

function deleteFlat(id) {
    if (confirm('Are you sure you want to delete this flat?')) {
        fetch(`/owner/flats/${id}`, {
            method: 'DELETE'
        })
...
function deleteBungalow(id) {
    if (confirm('Are you sure you want to delete this bungalow?')) {
        fetch(`/owner/bungalows/${id}`, {
            method: 'DELETE'
        })
        .then(response => response.json())
        .then(data => {
...
function openBungalowEditModal(id, city, location, address, status, price, image1, image2, image3, description) {
    document.getElementById('editBungalowId').value = id;
    document.getElementById('editCity').value = city;
    document.getElementById('editLocation').value = location;
    document.getElementById('editAddress').value = address;
    document.getElementById('editStatus').value = status;
    ...
    new bootstrap.Modal(document.getElementById('editBungalowModal')).show();
}
...
async function updateFlat() {
    ...
    try {
        const response = await fetch(`/owner/flats/${flatId}`, {
            method: 'PUT',
...
async function updateBungalow() {
    const form = document.getElementById('editBungalowForm');
    const formData = new FormData(form);
    ...
    const bungalowId = formData.get('id');

    try {
        const response = await fetch(`/owner/bungalows/${bungalowId}`, {
            method: 'PUT',
            body: formData // FormData will handle its own content type
        });
